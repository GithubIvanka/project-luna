    ...
    pub fn parse(bytes: &[u8]) -> BootResult<Self> {
        ...
        for raw in text.lines() {
            ...
            match (section, key.trim()) {
                ...
                ("image", "role") => role = match parse_string(value).as_deref() {
        ...
        if result.format != "squashfs" || result.arch != "x86_64" || result.compatible_kernels.is_empty() {
...
impl BootCatalog {
    pub fn discover(fs: &mut SystemFilesystem) -> BootResult<Self> {
        ...
        for image in images.iter().filter(|entry| entry.is_file() && entry.name.ends_with(".squashfs")) {
            let stem = &image.name[..image.name.len() - ".squashfs".len()];
            let manifest_path = format!("/images/{}.toml", stem);
            let Ok(manifest_bytes) = fs.read_file(&manifest_path) else { continue; };
            let Ok(manifest) = ImageManifest::parse(&manifest_bytes) else { continue; };
            ...
            let mut target = BootTarget::new(
                match manifest.role {
                    ...
                    ImageRole::Factory => "Factory Environment".to_owned(),
                    ImageRole::Recovery => "Recovery Environment".to_owned(),
                },
            ...
            let mode = match manifest.role {
                ImageRole::Normal => "normal",
                ImageRole::Factory => "factory",
                ImageRole::Recovery => "recovery",
            };
            target = target.with_cmdline(format!(
                ...
                image.name, kernel.version, mode
            ));
            match manifest.role {
        ...
        let default_target = if targets.is_empty() { 0 } else { 0 };
        Ok(Self { targets, recovery, factory, default_target })
    }
}

fn select_kernel(manifest: &ImageManifest, kernels: &[KernelRecord]) -> Option<KernelRecord> {
    kernels.iter()
        .filter(|kernel| manifest.compatible_kernels.iter().any(|allowed| {
            allowed == "*" || allowed == &kernel.version || (allowed == "default" && kernels.len() == 1)
        }))
...
fn find_file(fs: &mut SystemFilesystem, paths: &[String]) -> BootResult<Option<String>> {
    for path in paths {
        if fs.read_file(path).is_ok() { return Ok(Some(path.clone())); }
    }
    Ok(None)
}
...
fn parse_string_array(value: &str) -> Vec<String> {
    let value = value.trim();
    if !value.starts_with('[') || !value.ends_with(']') { return Vec::new(); }
    value[1..value.len() - 1].split(',').filter_map(parse_string).collect()
}
