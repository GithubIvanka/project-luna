...
impl BootMenu {
        ...
        loop {
            ...
            match self.stdin.read_key() {
                ...
                Ok(Some(Key::Printable(c))) if c == '\r' || c == '\n' => match entries[selected] {
                    MenuEntry::SystemImageSelection => {
                        if let Some(result) = self.select_system_image(targets, default_target) {
                            return Some(result);
                        }
                    }
    ...
    fn selection_for(entry: MenuEntry, default_target: usize) -> BootSelection {
        match entry {
            MenuEntry::Continue | MenuEntry::SystemImageSelection => BootSelection {
                action: BootMenuAction::Continue,
                target_index: default_target,
            },
