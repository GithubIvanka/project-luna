...
impl DirEntry {
    ...
    pub fn is_file(&self) -> bool {
        self.file_type == EXT4_FT_REG_FILE || self.file_type == 0
    }
}
...
impl<D: BlockDevice> Ext4<D> {
    pub fn open(mut device: D) -> BootResult<Self> {
        ...
        if u16_at(&sb, 0x38) != EXT4_SUPER_MAGIC {
            return Err(BootError::InvalidFilesystem);
        }
        ...
        if descriptor_size < 32 || descriptor_size as u32 > block_size {
            return Err(BootError::InvalidFilesystem);
        }
        ...
        Ok(Self {
            ...
            geometry: Ext4Geometry {
                ...
                inode_count,
                descriptor_size,
                groups: groups.min(u32::MAX as u64) as u32,
            },
            has_64bit,
        })
    }
    ...
    pub fn read_dir(&mut self, path: &str) -> BootResult<Vec<DirEntry>> {
        ...
        while off + 8 <= data.len() {
            ...
            if rec_len < 8 || off + rec_len > data.len() || name_len > rec_len - 8 {
                return Err(BootError::InvalidFilesystem);
            }
            if ino != 0 && name_len != 0 {
    ...
    fn group_descriptor(&mut self, group: u32) -> BootResult<Vec<u8>> {
        if group >= self.geometry.groups {
            return Err(BootError::FilesystemError);
        }
        let table_block = if self.geometry.block_size == 1024 {
            2
        } else {
            1
        };
        let offset = table_block as u64 * self.geometry.block_size as u64
            + group as u64 * self.geometry.descriptor_size as u64;
    ...
    fn inode_table_block(&mut self, group: u32) -> BootResult<u64> {
        ...
        let hi = if self.has_64bit && desc.len() >= 44 {
            u32_at(&desc, 40) as u64
        } else {
            0
        };
        Ok(lo | (hi << 32))
    ...
    fn read_inode(&mut self, ino: u32) -> BootResult<Inode> {
        if ino == 0 || ino > self.geometry.inode_count {
            return Err(BootError::FilesystemError);
        }
        let index = ino - 1;
        ...
        let offset = table * self.geometry.block_size as u64
            + local as u64 * self.geometry.inode_size as u64;
    ...
    fn find_in_directory(&mut self, dir: &Inode, wanted: &str) -> BootResult<u32> {
        let mut data =
            vec![0u8; usize::try_from(dir.size).map_err(|_| BootError::FilesystemError)?];
        ...
        while off + 8 <= data.len() {
            ...
            if ino != 0
                && name == wanted
                && (file_type == EXT4_FT_DIR || file_type == EXT4_FT_REG_FILE || file_type == 0)
            {
                return Ok(ino);
            }
            off += rec_len;
        ...
        if depth == 0 {
            for i in 0..entries {
                ...
                let extent_end = logical
                    .checked_add(len)
                    .ok_or(BootError::InvalidFilesystem)?;
                if wanted_block < logical || wanted_block >= extent_end {
                    continue;
                }
                if raw_len & 0x8000 != 0 {
                    out.fill(0);
    ...
    fn read_legacy_block_range(
        ...
        if block_index >= 12 {
            return Err(BootError::Unsupported("ext4 indirect block maps"));
        }
...
fn bs<D: BlockDevice>(fs: &Ext4<D>) -> u64 {
    fs.geometry.block_size as u64
}
