...
//! regular-file lookup and reads. Journaling, writes, checksums and extended
...
impl<D: BlockDevice> Ext4<D> {
    pub fn open(mut device: D) -> BootResult<Self> {
        ...
        let groups = (blocks.saturating_sub(first_data_block as u64)
            + blocks_per_group as u64 - 1)
    ...
    fn find_in_directory(&mut self, dir: &Inode, wanted: &str) -> BootResult<u32> {
        ...
        while off + 8 <= data.len() {
            ...
            if ino != 0 && name == wanted {
                if file_type == EXT4_FT_DIR || file_type == 1 || file_type == 0 {
                    return Ok(ino);
                }
            }
            off += rec_len;
    ...
    fn read_inode_data(&mut self, inode: &Inode, out: &mut [u8]) -> BootResult<()> {
        if inode.flags & EXT4_EXTENTS_FL != 0 {
            self.read_extent_node(&inode.blocks, out, 0)?;
        } else {
            self.read_legacy_blocks(&inode.blocks, out)?;
        }
        Ok(())
    }
