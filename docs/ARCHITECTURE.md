...
`images/` содержит System Images и их соседние manifests. **Краткая модель:** `SYS = boot/runtime core + default config`. System Image — непосредственно `luna-X.Y.Z.squashfs`. Внутри System Image находится минимальная неизменяемая база ОС: системные приложения, необходимые библиотеки, драйверы, отдельный каталог firmware, configuration defaults и неизменяемые ресурсы. `drivers/` и `firmware/` — разные классы ресурсов и не объединяются в один каталог. Внешние upstream-компоненты, необходимые для работы ОС, могут входить в DATA/provider layers; compositor/shell не являются содержимым System Image.
...
`kernels/` содержит независимые версионируемые ядра Linux. `config/` содержит boot state и `luna-data.toml`. Этот манифест хранит GUID физического диска и GUID раздела целевого `LUNA-DATA` для быстрого подключения. `recovery/` содержит единственный Recovery DATA Image и его manifest: `recovery.squashfs` и `recovery.toml`. Этот singleton используется совместимыми System Images; отдельного Recovery System Image нет.
...
`system/` содержит управляемые ОС изменяемые расширения и ресурсы. Его структура намеренно повторяет верхнеуровневую классификацию System Image: `apps/`, `drivers/`, `firmware/`, `libs/`, `config/` и `resources/` с одинаковыми типами ресурсов. `users/` содержит пользовательские данные; `cache/` не является durable storage.
...
System Image — **не вся ОС Luna**, а неизменяемая, версионированная минимальная userspace-база конкретного релиза. Он содержит необходимый immutable minimum: системные программы, библиотеки, драйверы, firmware, конфигурацию по умолчанию и системные ресурсы (`fonts/`, `icons/`, `themes/`, `cursors/`, `sounds/`, `locales/`, `translations`), достаточные для полноценного запуска ОС даже при отсутствии физической `LUNA-DATA`. `luna-init` использует System Image как источник файловой системы и материализует работающую системную среду в логическом `/` на основе RAM.

`LUNA-DATA` — отдельная **изменяемая и расширяемая часть ОС**. Наличие DATA не является условием для того, чтобы immutable System Image мог предоставить базовый полноценный запуск; после подключения DATA она предоставляет установленные Bundles, изменяемую конфигурацию, состояние, дополнительные системные компоненты и управляемые ресурсы. Канонический payload:
...
После инициализации ядра первым процессом пользовательского пространства становится `luna-init` и получает PID 1.
...
## 16. Физическая организация компонентов

Каталог `components/` разделён по архитектурной роли:
...
`core/` содержит основу ОС и runtime contracts. `system/` содержит системные приложения и административные инструменты. `apps/` предназначен для обычных пользовательских приложений. `external/providers/` содержит Luna-facing adapters внешних providers. `external/libraries/` предназначен для дополнительных сторонних/support libraries.

Эта физическая группировка не создаёт новых runtime layers: несколько crates внутри одной группы могут работать как один процесс или библиотечная группа.

## 17. Исходные документы
...
## 18. Правила разработки с участием AI
