...
Базовые системные приложения являются частью System Image. Их базовая версия не хранится в DATA.

Обновление системного приложения происходит только как часть обновления соответствующего System Image. Обновление System Image тем самым является механизмом обновления базовых системных приложений.

`LUNA-DATA/system` не предоставляет заменяющую копию системного приложения. DATA может содержать дополнительные системные компоненты, но они не меняют ownership базовых приложений System Image и не превращают обновление отдельного системного приложения в самостоятельный DATA-механизм.
...
Базовые системные приложения являются исключением из общего правила DATA priority: они принадлежат System Image и не имеют DATA replacement path.
...
Базовые системные приложения не относятся к DATA-replaceable сущностям.
...
Этот же принцип применяется к библиoteкам, драйверам, firmware и прочим DATA-replaceable ресурсам с учётом их отдельных compatibility/security policies.
...
Базовые системные приложения поставляются только в System Image и являются частью его версии.

Обновление базовых системных приложений выполняется только через выпуск/активацию новой версии System Image.

DATA не содержит replacement-копий базовых системных приложений и не является механизмом их независимого обновления.

Поэтому наличие новой версии System Image является тем фактором, который меняет версию базовых системных приложений Luna.
