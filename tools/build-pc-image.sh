...
if [ -z "$KERNEL" ]; then
    KERNEL="$(find_latest '/boot/vmlinuz-*' || true)"
fi
if [ -z "$BUSYBOX" ]; then
...
if ! rustup target list --installed | grep -qx "$RUNTIME_TARGET"; then
    echo "Installing Rust target $RUNTIME_TARGET" >&2
...
file "$RUNTIME" | grep -q 'statically linked' || {
    echo "luna-system-runtime is not statically linked; refusing to build PC image" >&2
    exit 1
}
...
for applet in sh ls cat mount umount ps pwd echo clear hostname dmesg mkdir rm; do
    ln -sf busybox "$SYSTEM_ROOT/bin/$applet"
done

cp "$RUNTIME" "$SYSTEM_ROOT/sbin/luna-system-runtime"
chmod 0755 "$SYSTEM_ROOT/sbin/luna-system-runtime"
ln -sf luna-system-runtime "$SYSTEM_ROOT/sbin/init"
...
cp "$BUSYBOX" "$INIT_ROOT/bin/busybox"
chmod 0755 "$INIT_ROOT/bin/busybox"
cp "$REPO_ROOT/boot/luna-boot/tests/ovmf/luna-init" "$INIT_ROOT/init"
...
sgdisk \
    ...
    -n 2:264192:+384M -t 2:8300 -c 2:SYSTEM \
    -n 3:1056768:+512M -t 3:8300 -c 3:DATA \
    "$OUT/luna-pc.img" >/dev/null
...
sha256sum \
    "$OUT/luna-pc.img" \
    "$OUT/luna-${LUNA_VERSION}.squashfs" \
