...
for tool in cargo rustup sgdisk mkfs.ext4 mkfs.fat mcopy mmd dd mksquashfs file truncate sha256sum; do
...
cp "$RUNTIME" "$SYSTEM_ROOT/sbin/luna-system-runtime"; chmod 0755 "$SYSTEM_ROOT/sbin/luna-system-runtime"
cp "$LOGIN" "$SYSTEM_ROOT/usr/bin/luna-login"; chmod 0755 "$SYSTEM_ROOT/usr/bin/luna-login"
