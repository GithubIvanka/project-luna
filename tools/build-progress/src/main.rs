...
impl Diagnostic {
    ...
    fn push_context_if_useful(&mut self, line: &str) {
        ...
        if value.trim().is_empty() || self.context.iter().any(|item| item == value) {
            return;
        }
...
fn matches_action(line: &str, patterns: &[String]) -> bool {
    patterns
        .iter()
        .any(|pattern| simple_pattern_match(pattern, line))
}
...
fn simple_pattern_match(pattern: &str, line: &str) -> bool {
    match pattern {
        p if p.contains("HOST)?") && p.contains("ZOFFSET") => {
            let trimmed = line.trim_start();
            [
                "CC ", "CXX ", "RUSTC ", "AR ", "LD ", "AS ", "OBJCOPY ", "OBJDUMP ", "STRIP ",
                "GEN ", "BUILD ", "BINDGEN ", "MODPOST ", "ZOFFSET ",
            ]
            .iter()
...
fn run(config: Config) -> io::Result<i32> {
    ...
    let mut diagnostics_started = false;
...
mod tests {
    ...
    fn rust_diagnostic_keeps_location_and_context() {
        let mut diagnostic =
            Diagnostic::from_start("warning: struct `TargetManager` is never constructed").unwrap();
        ...
        assert!(
            diagnostic
                .context
                .iter()
                .any(|line| line.contains("pub struct TargetManager"))
        );
    }
