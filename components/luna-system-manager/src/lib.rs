...
mod tests {
    ...
    fn system_state_keeps_atomic_current_factory_and_recovery_targets() {
        ...
        assert_eq!(
            state.recovery().target().image().version(),
            Version::new(2, 0, 0)
        );
        assert_eq!(
            state.recovery().target().init().version(),
            Version::new(2, 1, 0)
        );
        assert_eq!(
            state.recovery().target().kernel().version(),
            Version::new(8, 1, 0)
        );
    ...
    fn current_update_preserves_factory_and_recovery() {
        ...
        assert_eq!(
            manager.state().current().image().version(),
            Version::new(4, 0, 0)
        );
        assert_eq!(
            manager.state().current().init().version(),
            Version::new(4, 1, 0)
        );
        assert_eq!(
            manager.state().current().kernel().version(),
            Version::new(9, 0, 0)
        );
