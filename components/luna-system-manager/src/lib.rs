...
impl SystemTarget {
    pub const fn new(image: SystemImageRef, init: InitRef, kernel: KernelRef) -> Self {
        Self { image, init, kernel }
    }
    pub const fn image(&self) -> &SystemImageRef { &self.image }
    pub const fn init(&self) -> &InitRef { &self.init }
    pub const fn kernel(&self) -> &KernelRef { &self.kernel }
}
...
impl RecoveryTarget {
    ...
    pub const fn target(&self) -> &SystemTarget { &self.target }
    pub const fn data(&self) -> &RecoveryDataImageRef { &self.data }
}
...
impl SystemState {
    pub const fn new(current: SystemTarget, factory: SystemTarget, recovery: RecoveryTarget) -> Self {
        Self { current, factory, recovery }
    }
    pub const fn current(&self) -> &SystemTarget { &self.current }
    pub const fn factory(&self) -> &SystemTarget { &self.factory }
    pub const fn recovery(&self) -> &RecoveryTarget { &self.recovery }
}
...
impl<S: StateStore> PersistentSystemManager<S> {
    ...
    pub fn initialize(mut store: S, state: SystemState) -> Result<Self, SystemManagerError> {
        ...
        store.transaction(store.revision(), tx).map_err(state_error)?;
        Ok(Self { store, state })
    }
    pub fn state(&self) -> &SystemState { &self.state }
    pub fn revision(&self) -> Revision { self.store.revision() }
    pub fn store(&self) -> &S { &self.store }
    pub fn set_current(&mut self, image: Version, init: Version, kernel: Version, expected: Revision) -> Result<Revision, SystemManagerError> {
        let next = SystemState::new(
            SystemTarget::new(SystemImageRef::new(image), InitRef::new(init), KernelRef::new(kernel)),
            self.state.factory().clone(),
            self.state.recovery().clone(),
        );
        let tx = target_transaction(CURRENT_IMAGE, CURRENT_INIT, CURRENT_KERNEL, next.current());
...
impl PersistentSystemManager<RedbStateStore> {
    ...
    pub fn open_or_initialize_redb(data_root: impl AsRef<std::path::Path>, default_state: SystemState) -> Result<Self, SystemManagerError> {
...
impl<S: StateStore> SystemQuery for PersistentSystemManager<S> {
    ...
    fn state(&self) -> Result<SystemState, Self::Error> { Ok(self.state.clone()) }
}

fn encode_state(state: &SystemState) -> StateTransaction {
    let mut tx = target_transaction(CURRENT_IMAGE, CURRENT_INIT, CURRENT_KERNEL, state.current());
    tx.set(StateKey::new(FACTORY_IMAGE), StateValue::new(state.factory().image().version().to_string().into_bytes()))
      .set(StateKey::new(FACTORY_INIT), StateValue::new(state.factory().init().version().to_string().into_bytes()))
      .set(StateKey::new(FACTORY_KERNEL), StateValue::new(state.factory().kernel().version().to_string().into_bytes()))
      .set(StateKey::new(RECOVERY_IMAGE), StateValue::new(state.recovery().target().image().version().to_string().into_bytes()))
      .set(StateKey::new(RECOVERY_INIT), StateValue::new(state.recovery().target().init().version().to_string().into_bytes()))
      .set(StateKey::new(RECOVERY_KERNEL), StateValue::new(state.recovery().target().kernel().version().to_string().into_bytes()))
      .set(StateKey::new(RECOVERY_DATA), StateValue::new(state.recovery().data().version().to_string().into_bytes()));
    tx
}

fn target_transaction(image_key: &'static str, init_key: &'static str, kernel_key: &'static str, target: &SystemTarget) -> StateTransaction {
    let mut tx = StateTransaction::new();
    tx.set(StateKey::new(image_key), StateValue::new(target.image().version().to_string().into_bytes()))
      .set(StateKey::new(init_key), StateValue::new(target.init().version().to_string().into_bytes()))
      .set(StateKey::new(kernel_key), StateValue::new(target.kernel().version().to_string().into_bytes()));
    tx
}
...
fn read_target<S: StateStore>(store: &S, image_key: &'static str, init_key: &'static str, kernel_key: &'static str) -> Result<SystemTarget, SystemManagerError> {
    Ok(SystemTarget::new(
...
fn read_version<S: StateStore>(store: &S, key: &'static str) -> Result<Version, SystemManagerError> {
    let value = store.get(&StateKey::new(key)).map_err(state_error)?.ok_or(SystemManagerError::MissingState(key))?;
    let text = std::str::from_utf8(value.as_slice()).map_err(|_| SystemManagerError::InvalidVersion("non-utf8".into()))?;
...
fn parse_version(value: &str) -> Option<Version> {
    ...
    if parts.next().is_some() { None } else { Some(Version::new(major, minor, patch)) }
}
...
mod tests {
    use super::*;
    use luna_state::{MemoryStateStore, RedbStateStore};
    use std::time::{SystemTime, UNIX_EPOCH};

    fn target(image: (u32, u32, u32), init: (u32, u32, u32), kernel: (u32, u32, u32)) -> SystemTarget {
        SystemTarget::new(
    ...
    fn durable_system_state_round_trips() {
        ...
        let path = std::env::temp_dir().join(format!(
            "luna-system-manager-round-trip-{}-{}.redb",
            std::process::id(),
            SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos(),
        ));
        {
            let store = RedbStateStore::open(&path).unwrap();
            let manager = PersistentSystemManager::initialize(store, initial.clone()).unwrap();
    ...
    fn current_update_preserves_factory_and_recovery() {
        let initial = state();
        let mut manager = PersistentSystemManager::initialize(MemoryStateStore::new(), initial.clone()).unwrap();
        let revision = manager.revision();
        let next = manager.set_current(Version::new(4, 0, 0), Version::new(4, 1, 0), Version::new(9, 0, 0), revision).unwrap();
    ...
    fn stale_revision_does_not_change_state() {
        ...
        assert!(manager.set_current(Version::new(5, 0, 0), Version::new(5, 1, 0), Version::new(10, 0, 0), stale).is_err());
        assert_eq!(manager.state(), &initial);
    }

    #[test]
    fn missing_redb_state_is_initialized() {
        let root = std::env::temp_dir().join(format!("luna-system-manager-test-{}", std::process::id()));
