...
pub struct SystemImageRef { version: Version }
impl SystemImageRef { pub const fn new(version:Version)->Self{Self{version}} pub const fn version(&self)->Version{self.version} }

#[derive(Clone, Debug, Eq, PartialEq, Hash)]
pub struct KernelRef { version: Version }
impl KernelRef { pub const fn new(version:Version)->Self{Self{version}} pub const fn version(&self)->Version{self.version} }

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SystemState { current:SystemImageRef, factory:SystemImageRef, current_kernel:KernelRef, factory_kernel:KernelRef }
impl SystemState{
 pub const fn new(current:SystemImageRef,factory:SystemImageRef,current_kernel:KernelRef,factory_kernel:KernelRef)->Self{Self{current,factory,current_kernel,factory_kernel}}
 pub const fn current(&self)->&SystemImageRef{&self.current}
 pub const fn factory(&self)->&SystemImageRef{&self.factory}
 pub const fn current_kernel(&self)->&KernelRef{&self.current_kernel}
 pub const fn factory_kernel(&self)->&KernelRef{&self.factory_kernel}
}

pub trait SystemQuery{type Error;fn state(&self)->Result<SystemState,Self::Error>;}

#[derive(Debug)]
pub enum SystemManagerError { State(String), MissingState(&'static str), InvalidVersion(String) }
impl fmt::Display for SystemManagerError{fn fmt(&self,f:&mut fmt::Formatter<'_>)->fmt::Result{match self{Self::State(e)=>write!(f,"system state error: {e}"),Self::MissingState(k)=>write!(f,"missing system state key: {k}"),Self::InvalidVersion(v)=>write!(f,"invalid version in system state: {v}")}}}
...
fn read_state<S:StateStore>(store:&S)->Result<SystemState,SystemManagerError>{Ok(SystemState::new(SystemImageRef::new(read_version(store,CURRENT_IMAGE)?),SystemImageRef::new(read_version(store,FACTORY_IMAGE)?),KernelRef::new(read_version(store,CURRENT_KERNEL)?),KernelRef::new(read_version(store,FACTORY_KERNEL)?)))}
fn read_version<S:StateStore>(store:&S,key:&'static str)->Result<Version,SystemManagerError>{let value=store.get(&StateKey::new(key)).map_err(state_error)?.ok_or(SystemManagerError::MissingState(key))?;let text=std::str::from_utf8(value.as_slice()).map_err(|_|SystemManagerError::InvalidVersion("non-utf8".into()))?;parse_version(text).ok_or_else(||SystemManagerError::InvalidVersion(text.into()))}
fn parse_version(value:&str)->Option<Version>{let mut parts=value.split('.');let major=parts.next()?.parse().ok()?;let minor=parts.next()?.parse().ok()?;let patch=parts.next()?.parse().ok()?;if parts.next().is_some(){None}else{Some(Version::new(major,minor,patch))}}
fn state_error(error:luna_state::StateError)->SystemManagerError{SystemManagerError::State(error.to_string())}
