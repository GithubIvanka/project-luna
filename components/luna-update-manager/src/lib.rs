...
#[derive(Clone, Debug, Eq, PartialEq)] pub enum UpdateOperation { InstallSystemImage(Version), RemoveSystemImage(Version), InstallKernel(Version), RemoveKernel(Version), InstallApplication(BundleId,Version), RemoveApplication(BundleId,Version) }
#[derive(Clone, Debug, Eq, PartialEq)] pub struct UpdatePlan{operations:Vec<UpdateOperation>}
...
impl std::fmt::Display for UpdateError{fn fmt(&self,f:&mut std::fmt::Formatter<'_>)->std::fmt::Result{f.write_str(&self.0)}}impl std::error::Error for UpdateError{}
pub trait UpdateExecutor{fn execute(&mut self,plan:&UpdatePlan)->Result<(),UpdateError>}

/// Prototype executor that records an update plan as one atomic state
/// transaction. It does not mutate real system images or bundles yet.
pub struct TransactionalUpdateExecutor<'a,S:StateStore>{store:&'a mut S}
impl<'a,S:StateStore> TransactionalUpdateExecutor<'a,S>{pub fn new(store:&'a mut S)->Self{Self{store}} pub fn execute_at_revision(&mut self,expected:Revision,plan:&UpdatePlan)->Result<Revision,UpdateError>{plan.validate()?;let mut tx=StateTransaction::new();for (index,op) in plan.operations().iter().enumerate(){let key=StateKey::new(format!("update/plan/{index}"));let value=StateValue::new(format!("{op:?}").into_bytes());tx.set(key,value);}self.store.transaction(expected,tx).map_err(|e|UpdateError::new(e.to_string()))}}
