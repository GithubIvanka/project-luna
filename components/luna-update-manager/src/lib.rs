...
impl UpdateBackend for RecordingUpdateBackend {
    fn prepare(&mut self, _: &UpdatePlan) -> Result<(), UpdateError> {
        Ok(())
    }
    fn apply(&mut self, operation: &UpdateOperation) -> Result<(), UpdateError> {
    ...
    fn verify(&mut self, _: &UpdatePlan) -> Result<(), UpdateError> {
        Ok(())
    }
    fn rollback(&mut self, operation: &UpdateOperation) -> Result<(), UpdateError> {
...
impl<'a, S: StateStore, B: UpdateBackend> UpdateEngine<'a, S, B> {
    ...
    pub fn prepare(&mut self, id: &UpdateId, plan: &UpdatePlan) -> Result<Revision, UpdateError> {
        ...
        transaction
            ...
            .set(
                plan_key(id),
                StateValue::new(format_plan(plan).into_bytes()),
            )
            ...
            if let Err(error) = self.backend.apply(operation) {
                for completed in applied.iter().rev() {
                    if let Err(rollback_error) =
                        self.backend.rollback(&plan.operations()[*completed])
                    {
    ...
    pub fn reconcile_interrupted(
        ...
        if let Some(index) = self.read_inflight_index(id)? {
            indices.push(index);
        }
        indices.sort_unstable();
        indices.dedup();
        for index in indices.iter().rev() {
            let operation = plan.operations().get(*index).ok_or_else(|| {
                UpdateError::new("update journal references an invalid operation index")
            })?;
    ...
    fn read_applied_indexes(&self, id: &UpdateId) -> Result<Vec<usize>, UpdateError> {
        let value = self
            ...
            .get(&applied_key(id))
            .map_err(state_error)?
            .unwrap_or_else(|| StateValue::new(Vec::new()));
    ...
    fn read_inflight_index(&self, id: &UpdateId) -> Result<Option<usize>, UpdateError> {
        ...
        std::str::from_utf8(value.as_slice())
            .map_err(|_| UpdateError::new("update journal inflight index is not UTF-8"))?
            .parse::<usize>()
            .map(Some)
            .map_err(|_| UpdateError::new("update journal inflight index is invalid"))
    }
}
...
fn applied_key(id: &UpdateId) -> StateKey {
    StateKey::new(format!("updates/{}/applied", id.as_str()))
}
fn inflight_key(id: &UpdateId) -> StateKey {
    StateKey::new(format!("updates/{}/inflight", id.as_str()))
}
...
fn parse_indexes(value: &[u8]) -> Result<Vec<usize>, UpdateError> {
    ...
    std::str::from_utf8(value)
        .map_err(|_| UpdateError::new("update journal applied indexes are not UTF-8"))?
        .split(',')
        .filter(|entry| !entry.is_empty())
        .map(|entry| {
            entry
                .parse::<usize>()
                .map_err(|_| UpdateError::new("update journal applied index is invalid"))
        })
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn engine_records_commit_and_clears_inflight() {
        let mut store = MemoryStateStore::new();
        ...
        assert_eq!(
            store.get(&inflight_key(&id)).unwrap().unwrap().as_slice(),
            b""
        );
    }
    #[test]
    ...
    #[test]
    fn interrupted_operation_rolls_back_inflight_and_applied_steps() {
        let mut store = MemoryStateStore::new();
        let mut backend = RecordingUpdateBackend::default();
        let id = UpdateId::new("test-interrupted").unwrap();
        let mut plan = UpdatePlan::new();
        plan.push(UpdateOperation::InstallKernel(Version::new(9, 0, 0)));
        plan.push(UpdateOperation::InstallSystemImage(Version::new(2, 0, 0)));
        {
            let mut engine = UpdateEngine::new(&mut store, &mut backend);
            engine.prepare(&id, &plan).unwrap();
            engine.checkpoint(&id).unwrap();
            engine.set_phase(&id, UpdatePhase::Applying).unwrap();
            engine.backend.apply(&plan.operations()[0]).unwrap();
            engine.set_progress(&id, &[0], Some(1)).unwrap();
        }
        assert!(
            UpdateEngine::new(&mut store, &mut backend)
                .reconcile_interrupted(&id, &plan)
                .is_ok()
        );
        assert!(backend.applied().is_empty());
    }
