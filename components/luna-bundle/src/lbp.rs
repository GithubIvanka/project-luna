...
impl SectionKind {
    ...
    fn from_code(value: u32) -> Result<Self, LbpError> {
        match value {
            ...
            _ => Err(LbpError::UnknownSectionType(value)),
        }
    }
}
    ...
    pub fn validate(&self) -> Result<(), LbpError> {
        ...
        if self.bundle.id.trim().is_empty() || self.bundle.name.trim().is_empty() {
            return Err(LbpError::ManifestFormat(
                "bundle id/name must not be empty".into(),
            ));
        }
...
impl std::fmt::Display for LbpError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::UnsupportedVersion(v) => write!(f, "unsupported LBP version: {v}"),
            Self::UnsupportedFlags(v) => write!(f, "unsupported LBP flags: {v:#x}"),
...
mod tests {
    ...
    fn manifest() -> LbpManifest {
        LbpManifest {
            ...
            capabilities: Capabilities {
                requested: vec!["network".into()],
            },
            mappings: vec![],
            metadata: Metadata {
                author: Some("Luna".into()),
    ...
    fn manifest_roundtrips_toml() {
        ...
        assert_eq!(m, LbpManifest::from_toml(&text).unwrap());
    }

    #[test]
    fn archive_roundtrip_is_deterministic() {
        let dir = fixture();
        let m = manifest();
        let first = build_from_directory(&m, &dir).unwrap();
        let second = build_from_directory(&m, &dir).unwrap();
    ...
    fn rejects_corrupt_header() {
        ...
        assert!(matches!(
            ...
            Err(LbpError::InvalidHeader)
        ));
        let _ = fs::remove_dir_all(dir);
    }

    #[test]
    fn rejects_payload_hash_corruption() {
        let dir = fixture();
        let mut bytes = build_from_directory(&manifest(), &dir).unwrap();
