...
/// Validates structural invariants independent of the `.lbp` transport.
pub fn validate_manifest(manifest: &BundleManifest) -> Result<(), BundleError> {
    ...
    for resource in manifest.resources() {
        ...
        if !logical.starts_with('/') || logical.ends_with('/') || logical.split('/').any(|p| p == "..") { return Err(BundleError::InvalidLogicalPath(logical.to_owned())); }
        if source.starts_with('/') || source.split('/').any(|p| p == "..") { return Err(BundleError::InvalidSourcePath(source.to_owned())); }
...
#[cfg(test)]
mod tests {
    use super::{validate_manifest, BundleKind, BundleManifest, BundleMetadata, BundleResource, BundleError};
    use luna_common::{BundleId, Version};
