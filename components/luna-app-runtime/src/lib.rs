...
impl ApplicationInstance {
    ...
    pub fn detach_process(&mut self) -> Option<ProcessId> {
        self.process.take()
    }
...
impl InMemoryApplicationRuntime {
    ...
    pub fn launch_for_session_with_runtime(
        &mut self,
        manifest: &BundleManifest,
        mapping: &MappingTable,
        session: &UserSession,
...
impl LinuxApplicationRuntime {
    ...
    {
        if !program.starts_with('/') || program.contains("..") || program.contains('\0') {
            return Err(RuntimeError::InvalidExecutable(program.to_owned()));
        }
        if session.state() != SessionState::Active {
            return Err(RuntimeError::SessionNotActive);
        }
        ...
        let runtime_request = AuthorizationRequest {
            principal: Principal::Application(manifest.metadata().id().clone()),
            resource: Resource::Runtime(runtime_spec.kind()),
            permission: Permission::Use,
        };
        InMemoryApplicationRuntime::require_allow(policy, &runtime_request)?;
        for request in requests {
            InMemoryApplicationRuntime::require_allow(policy, request)?;
        }
    ...
    pub fn terminate(
        ...
        let process = self
            ...
            .process()
            .ok_or(RuntimeError::NoProcess)?;
        let _status = runtime.terminate_supervised_process(process)?;
        self.processes.remove(&process);
        self.cleanup_root(process);
        let instance = self.model.instance_mut(id)?;
        instance.detach_process();
        match instance.state() {
            ...
            InstanceState::Stopping => instance.transition(InstanceState::Stopped),
            _ => Ok(()),
        }
    }
    pub fn reconcile(
    ...
    fn cleanup_root(&mut self, process: ProcessId) {
