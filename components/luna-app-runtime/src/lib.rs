...
impl ApplicationInstance {
    pub fn new(id: ApplicationInstanceId, application: BundleId, version: Version, session: SessionId) -> Self { Self::new_with_runtime(id, application, version, session, RuntimeSpec::default()) }
    pub fn new_with_runtime(id: ApplicationInstanceId, application: BundleId, version: Version, session: SessionId, runtime: RuntimeSpec) -> Self { Self { id, application, version, session, runtime, state: InstanceState::Starting, process: None } }
...
pub enum RuntimeError {
    InvalidTransition, InvalidBundle(String), Mapping(MappingError), Namespace(NamespaceError), Security(String), SessionNotActive,
    InstanceNotFound, Process(ProcessError), Supervisor(luna_system_runtime::RuntimeError), ProcessAlreadyAttached, NoProcess,
    InvalidExecutable(String), Staging(String), RuntimeMismatch { mapping: Option<RuntimeKind>, requested: RuntimeKind },
}
impl std::fmt::Display for RuntimeError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::InvalidTransition => f.write_str("invalid application instance transition"), Self::InvalidBundle(e) => write!(f,"invalid bundle: {e}"), Self::Mapping(e) => write!(f,"mapping error: {e}"), Self::Namespace(e) => write!(f,"namespace error: {e}"), Self::Security(e) => write!(f,"security authorization failed: {e}"), Self::SessionNotActive => f.write_str("session is not active"), Self::InstanceNotFound => f.write_str("application instance not found"), Self::Process(e) => write!(f,"process supervision failed: {e}"), Self::Supervisor(e) => write!(f,"system runtime operation failed: {e}"), Self::ProcessAlreadyAttached => f.write_str("application instance already has a process"), Self::NoProcess => f.write_str("application instance has no process"), Self::InvalidExecutable(e) => write!(f,"invalid application executable: {e}"), Self::Staging(e) => write!(f,"namespace staging failed: {e}"), Self::RuntimeMismatch { mapping, requested } => write!(f,"runtime mismatch: mapping={mapping:?}, requested={requested}"),
        }
    }
}
...
pub struct NamespacePreparation<'a> { pub namespace: &'a LinuxMountNamespace, pub root: &'a Path, pub base_root: &'a Path, pub mapping: &'a MappingTable, pub policy: &'a dyn PolicyAuthority, pub requests: &'a [AuthorizationRequest], pub runtime: RuntimeSpec }
#[derive(Debug)] pub struct PreparedApplicationNamespace { instance: ApplicationInstanceId, root: LogicalRoot }
...
#[derive(Default)] pub struct InMemoryApplicationRuntime { next_id: u128, instances: BTreeMap<ApplicationInstanceId, ApplicationInstance> }
impl InMemoryApplicationRuntime {
    ...
    fn validate_resources(manifest: &BundleManifest, mapping: &MappingTable, runtime: RuntimeSpec) -> Result<(), RuntimeError> {
        ...
        for resource in manifest.resources() { let logical = LogicalPath::new(resource.logical_path()).map_err(RuntimeError::Mapping)?; mapping.resolve(&logical).map_err(|_| RuntimeError::Mapping(MappingError::NotMapped))?; }
        mapping.materialize().map_err(RuntimeError::Mapping)?; Ok(())
    }
    pub fn launch_for_session(&mut self, manifest: &BundleManifest, mapping: &MappingTable, session: &UserSession) -> Result<ApplicationInstanceId, RuntimeError> { self.launch_for_session_with_runtime(manifest, mapping, session, RuntimeSpec::default()) }
    pub fn launch_for_session_with_runtime(&mut self, manifest: &BundleManifest, mapping: &MappingTable, session: &UserSession, runtime: RuntimeSpec) -> Result<ApplicationInstanceId, RuntimeError> {
        ...
        let mut instance = ApplicationInstance::new_with_runtime(id, manifest.metadata().id().clone(), manifest.metadata().version(), session.id(), runtime); instance.transition(InstanceState::Running)?; self.instances.insert(id, instance); Ok(id)
    }
    pub fn prepare_authorized_namespace_for_session(&self, instance: ApplicationInstanceId, preparation: NamespacePreparation<'_>) -> Result<PreparedApplicationNamespace, RuntimeError> {
        ...
        Self::require_allow(preparation.policy, &runtime_request)?; for request in preparation.requests { Self::require_allow(preparation.policy, request)?; }
    ...
    fn require_allow(policy: &dyn PolicyAuthority, request: &AuthorizationRequest) -> Result<(), RuntimeError> { match policy.authorize(request).map_err(|e| RuntimeError::Security(e.to_string()))? { Decision::Allow => Ok(()), Decision::Deny => Err(RuntimeError::Security(format!("denied: {request:?}"))), Decision::Ask => Err(RuntimeError::Security("authorization requires user confirmation".into())), Decision::Constrained { constraints } => Err(RuntimeError::Security(format!("constraint enforcement not supplied: {constraints:?}"))) } }
    fn validate_mapping_only(mapping: &MappingTable, runtime: RuntimeSpec) -> Result<(), RuntimeError> { if !mapping.accepts_runtime(runtime.kind()) { return Err(RuntimeError::RuntimeMismatch { mapping: mapping.runtime(), requested: runtime.kind() }); } mapping.materialize().map_err(RuntimeError::Mapping)?; Ok(()) }
...
impl ApplicationRuntime for InMemoryApplicationRuntime {
    ...
    fn launch(&mut self, manifest: &BundleManifest, mapping: &MappingTable) -> Result<ApplicationInstance, Self::Error> { Self::validate_resources(manifest, mapping, RuntimeSpec::default())?; let id = ApplicationInstanceId::new(self.next_id); self.next_id = self.next_id.saturating_add(1); let mut instance = ApplicationInstance::new(id, manifest.metadata().id().clone(), manifest.metadata().version(), SessionId::new(0)); instance.transition(InstanceState::Running)?; self.instances.insert(id, instance.clone()); Ok(instance) }
...
impl LinuxApplicationRuntime {
    ...
    where I: IntoIterator<Item = S>, S: AsRef<std::ffi::OsStr> {
        ...
        InMemoryApplicationRuntime::require_allow(policy, &runtime_request)?; for request in requests { InMemoryApplicationRuntime::require_allow(policy, request)?; }
        fs::create_dir_all(staging_parent).map_err(|e| RuntimeError::Staging(e.to_string()))?;
        let id = ApplicationInstanceId::new(self.model.next_id); self.model.next_id = self.model.next_id.saturating_add(1); let root = staging_parent.join(format!("instance-{}", id.get()));
        ...
        let process = runtime.spawn_process_with_pre_exec(program, args, move || { let logical = namespace.materialize_logical_root(&root_for_child, &base_root, &mapping).map_err(|e| std::io::Error::other(e.to_string()))?; namespace.enter_logical_root(&logical).map_err(|e| std::io::Error::other(e.to_string()))?; Ok(()) });
        ...
        instance.attach_process(process)?; instance.transition(InstanceState::Running)?; self.model.instances.insert(id, instance); self.processes.insert(process, id); self.roots.insert(process, root); Ok(id)
    }
    ...
    pub fn terminate(&mut self, id: ApplicationInstanceId, runtime: &mut SystemRuntimeService) -> Result<(), RuntimeError> {
        ...
        let instance = self.model.instance_mut(id)?; instance.detach_process(); match instance.state() { InstanceState::Running => { instance.transition(InstanceState::Stopping)?; instance.transition(InstanceState::Stopped) }, InstanceState::Stopping => instance.transition(InstanceState::Stopped), _ => Ok(()) }
    }
