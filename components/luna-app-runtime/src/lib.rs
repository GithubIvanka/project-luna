...
pub struct InMemoryApplicationRuntime {
    next_id: u128,
    instances: BTreeMap<ApplicationInstanceId, ApplicationInstance>,
}
...
impl InMemoryApplicationRuntime {
    ...
    /// Validate and materialize the application namespace without crossing the
    /// final process-exec boundary. Process creation belongs to the system
    /// runtime/supervisor and is intentionally not hidden inside this crate.
    pub fn prepare_namespace_for_session(
        &self,
        instance: ApplicationInstanceId,
        ...
        let logical_root = namespace
            .materialize_logical_root(root, base_root, mapping)
            .map_err(RuntimeError::Namespace)?;
        Ok(PreparedApplicationNamespace {
            instance,
            root: logical_root,
        })
    }

    fn validate_mapping_only(mapping: &MappingTable) -> Result<(), RuntimeError> {
        mapping.materialize().map_err(RuntimeError::Mapping)?;
        Ok(())
    }
    ...
    pub fn fail(&mut self, id: ApplicationInstanceId) -> Result<(), RuntimeError> {
        self.instances
            .get_mut(&id)
            .ok_or(RuntimeError::InstanceNotFound)?
            .transition(InstanceState::Failed)
    }
}
...
mod tests {
    use super::{
        ApplicationInstance, ApplicationInstanceId, InMemoryApplicationRuntime, InstanceState,
    };
    ...
    fn setup() -> (BundleManifest, MappingTable, UserSession) {
        ...
        mapping
            .insert(MappingRule::new(
                ...
                PhysicalPath::new("/data/system/apps/example/bin/app"),
            ))
            .unwrap();
    ...
    fn runtime_requires_active_session_and_valid_mapping() {
        let (manifest, mapping, session) = setup();
        let mut runtime = InMemoryApplicationRuntime::new();
        let id = runtime
            .launch_for_session(&manifest, &mapping, &session)
            .unwrap();
        assert_eq!(runtime.instance(id).unwrap().state(), InstanceState::Running);
        runtime.stop(id).unwrap();
        assert_eq!(runtime.instance(id).unwrap().state(), InstanceState::Stopped);
    }
}
