...
impl StateStore for MemoryStateStore {
    ...
    fn set(&mut self, key: StateKey, value: StateValue) -> Result<(), StateError> {
        ...
        tx.set(key, value);
        self.transaction(revision, tx).map(|_| ())
    }
    fn remove(&mut self, key: &StateKey) -> Result<(), StateError> {
        let revision = self.revision;
        let mut tx = StateTransaction::new();
...
impl RedbStateStore {
    ...
    fn initialize_tables(&mut self) -> Result<(), StateError> {
        ...
        {
            tx.open_table(VALUES).map_err(storage_error)?;
            tx.open_table(META).map_err(storage_error)?;
        }
...
impl StateStore for RedbStateStore {
    ...
    fn transaction(
        ...
        {
            let mut values = tx.open_table(VALUES).map_err(storage_error)?;
...
mod tests {
    ...
    fn round_trip() {
        ...
        store
            .set(key.clone(), StateValue::new(b"v".to_vec()))
            .unwrap();
    ...
    fn atomic_revision() {
        ...
        let mut tx = StateTransaction::new();
        tx.set(StateKey::new("a"), StateValue::new(b"1".to_vec()))
            .set(StateKey::new("b"), StateValue::new(b"2".to_vec()));
        assert_eq!(store.transaction(revision, tx).unwrap(), revision.next());
    }
    #[test]
    fn stale_is_rejected() {
        ...
        store
            ...
            .unwrap();
        let mut tx = StateTransaction::new();
        tx.set(StateKey::new("a"), StateValue::new(b"stale".to_vec()));
        assert!(matches!(
            store.transaction(stale, tx),
    ...
    fn durable_store_survives_reopen() {
        let path = temp_db_path();
        {
            ...
            let mut tx = StateTransaction::new();
            tx.set(
                StateKey::new("system/boot/current"),
                StateValue::new(b"luna-1.0.0".to_vec()),
            )
            .set(
                StateKey::new("system/update/checkpoint"),
                StateValue::new(b"checkpointed".to_vec()),
            );
            assert_eq!(
                store.transaction(Revision::initial(), tx).unwrap(),
        ...
        }
        {
            ...
            assert_eq!(
                store
                    .get(&StateKey::new("system/boot/current"))
                    .unwrap()
                    .unwrap()
                    .as_slice(),
                b"luna-1.0.0"
            ...
            assert_eq!(
                store
                    .get(&StateKey::new("system/update/checkpoint"))
                    .unwrap()
                    .unwrap()
                    .as_slice(),
                b"checkpointed"
