...
impl SystemRuntimeService{
 ...
 pub fn create_login_session(&mut self,user:luna_common::UserId)->Result<SessionId,RuntimeError>{let id=SessionId::new(self.next_session_id);self.next_session_id=self.next_session_id.saturating_add(1);let mut session=UserSession::new(id,user);session.transition(SessionState::Authenticating).map_err(|e|RuntimeError::Session(e.to_string()))?;self.sessions.insert(id,session);Ok(id)}
 pub fn authenticate_session(&mut self,id:SessionId)->Result<(),RuntimeError>{let session=self.sessions.get_mut(&id).ok_or(RuntimeError::UnknownSession(id))?;session.transition(SessionState::Active).map_err(|e|RuntimeError::Session(e.to_string()))}
 pub fn cancel_login(&mut self,id:SessionId)->Result<(),RuntimeError>{let session=self.sessions.get_mut(&id).ok_or(RuntimeError::UnknownSession(id))?;session.transition(SessionState::Ending).map_err(|e|RuntimeError::Session(e.to_string()))?;session.transition(SessionState::Ended).map_err(|e|RuntimeError::Session(e.to_string()))}
 pub fn create_session(&mut self,user:luna_common::UserId)->Result<SessionId,RuntimeError>{let id=SessionId::new(self.next_session_id);self.next_session_id=self.next_session_id.saturating_add(1);let mut session=UserSession::new(id,user);session.transition(SessionState::Authenticating).map_err(|e|RuntimeError::Session(e.to_string()))?;session.transition(SessionState::Active).map_err(|e|RuntimeError::Session(e.to_string()))?;self.sessions.insert(id,session);Ok(id)}
 ...
 pub fn launch_session_shell(&mut self,id:SessionId,shell:&str)->Result<ProcessId,RuntimeError>{let session=self.session(id)?;if session.state()!=SessionState::Active{return Err(RuntimeError::Session("shell process requires an active session".into()))}let process=self.supervisor.spawn_inherited(shell,std::iter::empty::<&str>())?;self.session_processes.insert(process,id);Ok(process)}
 pub fn launch_graphical_session<I,S>(&mut self,id:SessionId,program:&str,args:I)->Result<ProcessId,RuntimeError> where I:IntoIterator<Item=S>,S:AsRef<std::ffi::OsStr>{let session=self.session(id)?;if session.state()!=SessionState::Active{return Err(RuntimeError::Session("graphical session requires an authenticated active UserSession".into()))}let process=self.supervisor.spawn(program,args)?;self.session_processes.insert(process,id);Ok(process)}
 pub fn launch_niri_session(&mut self,id:SessionId)->Result<ProcessId,RuntimeError>{self.launch_graphical_session(id,"niri-session",std::iter::empty::<&str>())}
 ...
 fn end_session(&mut self,id:SessionId)->Result<(),RuntimeError>{let session=self.sessions.get_mut(&id).ok_or(RuntimeError::UnknownSession(id))?;match session.state(){SessionState::Active|SessionState::Restricted=>{session.transition(SessionState::Ending).map_err(|e|RuntimeError::Session(e.to_string()))?;session.transition(SessionState::Ended).map_err(|e|RuntimeError::Session(e.to_string()))?;}SessionState::Ending=>{session.transition(SessionState::Ended).map_err(|e|RuntimeError::Session(e.to_string()))?;}_=>{}}Ok(())}
}
...
#[cfg(test)]mod tests{use super::{session_event_type,ProcessState,ProcessSupervisor,SystemRuntime,SystemRuntimeService};use luna_common::UserId;use luna_user_session::SessionState;#[test]fn runtime_has_stable_session_event_name(){assert_eq!(session_event_type().as_str(),"session.state.changed");}#[test]fn supervisor_spawns_and_reaps_real_child(){let mut s=ProcessSupervisor::new();let id=s.spawn("true",std::iter::empty::<&str>()).unwrap();let mut exited=false;for _ in 0..50{match s.poll(id).unwrap(){ProcessState::Running=>std::thread::sleep(std::time::Duration::from_millis(5)),ProcessState::Exited(status)=>{assert!(status.success());exited=true;break;}}}assert!(exited);assert!(s.is_empty());}#[test]fn supervisor_terminates_long_lived_child(){let mut s=ProcessSupervisor::new();let id=s.spawn("sleep",["60"]).unwrap();let status=s.terminate(id).unwrap();assert!(!status.success());assert!(s.is_empty());}#[test]fn system_runtime_owns_session_and_process_lifecycle(){let mut rt=SystemRuntimeService::new();rt.start();let session=rt.create_session(UserId::from("luna")).unwrap();let process=rt.launch_session_shell(session,"true").unwrap();for _ in 0..50{rt.supervise().unwrap();if rt.session(session).unwrap().state()==SessionState::Ended{break;}std::thread::sleep(std::time::Duration::from_millis(5));}assert_eq!(rt.session(session).unwrap().state(),SessionState::Ended);assert!(!rt.supervisor.contains(process));}#[test]fn generic_process_survives_session_supervision(){let mut rt=SystemRuntimeService::new();rt.start();let process=rt.spawn_process("true",std::iter::empty::<&str>()).unwrap();let mut exited=false;for _ in 0..50{rt.supervise().unwrap();match rt.poll_process(process){Ok(ProcessState::Running)=>std::thread::sleep(std::time::Duration::from_millis(5)),Ok(ProcessState::Exited(status))=>{assert!(status.success());exited=true;break;},Err(super::RuntimeError::Process(super::ProcessError::Unknown(id)))=>{panic!("generic supervised process disappeared before it could be observed: {}",id.get());},Err(error)=>panic!("generic supervised process polling failed: {error}")}}assert!(exited);}#[test]fn login_session_requires_authentication(){let mut rt=SystemRuntimeService::new();rt.start();let id=rt.create_login_session(UserId::from("alice")).unwrap();assert_eq!(rt.session(id).unwrap().state(),SessionState::Authenticating);rt.authenticate_session(id).unwrap();assert_eq!(rt.session(id).unwrap().state(),SessionState::Active);}}