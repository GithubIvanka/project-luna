...
impl SystemRuntimeService{
 ...
 /// Create a UserSession at the graphical login/authentication boundary.
 /// Authentication is represented by the session state rather than by a
 /// separate login-user process.
 ...
 pub fn launch_session_shell(&mut self,id:SessionId,shell:&str)->Result<ProcessId,RuntimeError>{let session=self.session(id)?;if session.state()!=SessionState::Active{return Err(RuntimeError::Session("graphical or shell process requires an active session".into()))}let process=self.supervisor.spawn_inherited(shell,std::iter::empty::<&str>())?;self.session_processes.insert(process,id);Ok(process)}
 /// Launch the graphical compositor/session for an authenticated UserSession.
 /// No TTY is used: GUI processes use detached stdio and are supervised by the
 /// same system-wide ProcessSupervisor as every other runtime process.
