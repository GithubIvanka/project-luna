...
/// Identifies the owner/scope of a configuration value.
...
/// A configuration key inside a scope.
...
/// Untyped configuration value at the storage boundary.
///
/// Higher layers can interpret this value as TOML data or another format. The
/// foundation contract intentionally does not bake a serializer into the API.
...
/// Storage interface for one configuration source.
...
/// In-memory store used for contract tests and higher-layer composition.
...
///
/// For application settings the precedence is:
///
/// 1. user/application override;
/// 2. application-provided default;
/// 3. system default.
#[derive(Clone, Debug, Default)]
